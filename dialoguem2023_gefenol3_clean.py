...
def leer_datos_ronda_1():
    ...
    try:
        # Obtén la lista de todos los nombres de archivos en la carpeta de
        # avatares
    ...
    # Extrae los nombres de los avatares de los nombres de los archivos
    # (eliminando la extensión .png)
    all_avatars = [
        os.path.splitext(file)[0]
        for file in avatar_files if file.endswith('.png')
    ]

    if not avatar_files:
        print(
            'Error: No se encontraron archivos de avatares con extensión .png '
            'en la carpeta "Avatares".'
        )
        return None, []

    avatars = [
        avatar for avatar in all_avatars
        if os.path.exists(f'data/round_1/{avatar}.csv')
    ]
    print(avatars)

    df_ronda = pd.DataFrame(index=avatars, columns=avatars)

    # Diccionario para almacenar las opiniones únicas de cada avatar en rondas
    # anteriores
    ...
    df_autopuntuaciones = df_autopuntuaciones[
        df_autopuntuaciones['avatar'].isin(avatars)
    ]
    df_autopuntuaciones.set_index('avatar', inplace=True)

    for avatar, opinion, _ in zip(
            df_autopuntuaciones.index, df_autopuntuaciones['opinion'],
            df_autopuntuaciones['autopuntuacion']):
    ...
    for avatar in avatars:
        ...
        if avatar not in df_autopuntuaciones.index:
            ...
            df_ronda.loc[avatar, avatar] = df_autopuntuaciones.loc[
                avatar, 'autopuntuacion'
            ]

        # Leer las puntuaciones que el avatar ha dado a los demás
        df_avatar = pd.read_csv(
            f'data/round_1/{avatar}.csv',
            names=['avatar', 'no_sirve', 'position']
        )
        for evaluated_avatar in avatars:
            ...
            position_df = df_avatar[
                df_avatar['avatar'] == evaluated_avatar
            ]['position']

            if position_df.empty:
                # Valor predeterminado si no se encuentra el avatar evaluado.
                position = -1
            else:
...
def leer_datos_ronda_2(opiniones_ronda_1, df_ronda_1):
    ...
    df_autopuntuaciones = df_autopuntuaciones.reindex(avatars).reset_index()

    # 2.1 Crear el diccionario opiniones_ronda_2 con las opiniones
    opiniones_ronda_2 = df_autopuntuaciones[['avatar', 'opinion']]
    opiniones_ronda_2 = opiniones_ronda_2.set_index('avatar')
    opiniones_ronda_2 = opiniones_ronda_2.to_dict()['opinion']

    # 2.2 Actualizar df_ronda_2 con las autopuntuaciones
    for avatar in avatars:
        if pd.isna(df_autopuntuaciones.loc[
                    df_autopuntuaciones['avatar'] == avatar, 'autopuntuacion'
                ].iloc[0]):
            df_ronda_2.loc[avatar, avatar] = df_ronda_1.loc[avatar, avatar]
        else:
            df_ronda_2.loc[avatar, avatar] = df_autopuntuaciones.loc[
                df_autopuntuaciones['avatar'] == avatar, 'autopuntuacion'
            ].iloc[0]

    # 2.3 Comparar opiniones_ronda_2 con opiniones_ronda_1
    # y actualizar opiniones_ronda_2
    ...
    # 2.4 Comprobar si hay avatares en opiniones_ronda_1
    # que no están en opiniones_ronda_2 y añadirlos con valor None
    for avatar in avatars:
    ...
    for avatar_1 in avatars:
        # 3.1 Crear una lista evaluar_avatars
        # con todos los avatares menos avatar_1
        evaluar_avatars = [avatar for avatar in avatars if avatar != avatar_1]

        # 3.2 Leer el archivo de evaluaciones de avatar_1 si existe
        try:
            df_avatar = pd.read_csv(
                f'data/round_2/{avatar_1}.csv',
                names=['avatar', 'no_sirve', 'position']
            )

            for avatar_2 in avatars:
                position_df = df_avatar[df_avatar['avatar'] == avatar_2]
                position_df = position_df['position']
            ...
            print(
                'Advertencia: No se encontró el archivo para '
                f'el avatar "{avatar_1}" en la carpeta "round_2".'
            )

        # 3.3 Para cada avatar_2 en evaluar_avatars:
        for avatar_2 in evaluar_avatars:
            # 3.3.1 Comprobar si su opinión es "Same"
            # o None en opiniones_ronda_2
            if opiniones_ronda_2[avatar_2] in ['Same', None]:
                # 3.3.2 Si lo es, copiar la evaluación de avatar_1 a avatar_2
                # de df_ronda_1 a df_ronda_2
                df_ronda_2.loc[avatar_2, avatar_1] = df_ronda_1.loc[
                    avatar_2, avatar_1
                ]
...
def leer_compatibilidades(num_rondas, avatars_existentes):
    ...
    for i in range(1, num_rondas+1):
        ronda = f'ronda_{i}'
        avatars_ronda = [
            avatar for avatar in avatars_existentes
            if os.path.exists(f'data/round_{i}/{avatar}.csv')
        ]
        ...
        for avatar in avatars_ronda:
            ...
            df_compatibilidad = pd.read_csv(
                filepath, names=['avatar', 'no_sirve', 'compatible']
            )
            df_compatibilidad.set_index('avatar', inplace=True)

            compatibilidades[avatar][ronda] = df_compatibilidad[
                df_compatibilidad['compatible'] == 'yes'
            ].index.tolist()
...
def procesar_datos():
    ...
    # La lista de avatars existentes
    avatars_existentes = df_ronda_1.index.tolist()
    datos['compatibilidades'] = leer_compatibilidades(
        num_rondas, avatars_existentes
    )
...
def leer_datos_ronda_avatar(ronda, avatar):
    df_avatar = pd.read_csv(
        f'data/round_{ronda}/{avatar}.csv',
        names=['avatar', 'no_sirve', 'position']
    )
    # Se queda con la primera entrada para cada avatar
    df_avatar = df_avatar.groupby('avatar').first()
...
for ronda, df in datos['rondas'].items():
    ...
    sns.heatmap(
        df_sorted, cmap=cmap, annot=True, fmt='d', vmin=-1, vmax=10,
        cbar=False, ax=ax
    )
    ...
    plt.savefig(f'fig/{ronda}_puntuaciones.png', bbox_inches='tight')

# compatibles giraffe: ['dolphin', 'lion', 'elephant', 'cow', 'frog', 'monkey',
# 'bear', 'sheep', 'octopus']
# [10,9,5,9,6,1,5,7,6]
...
for ronda, df in datos['rondas'].items():
    ...
    # Convertir el diccionario en un DataFrame y transponer
    df_diff = pd.DataFrame(diff_dict).transpose()
    ...
    ax = sns.heatmap(
        df_diff, cmap=cmap, annot=True, fmt='d', vmin=-11, vmax=10, cbar=False
    )
    ...
    plt.savefig(f'fig/{ronda}_simetria.png', bbox_inches='tight')
...
for avatar in avatars_sorted:
    # Usa el diccionario para obtener el índice correcto
    i = index_dict[avatar]
    # Calcula la media de la puntuación que le han dado los otros
    # en la ronda_1 y en la ronda_2
...
for avatar in avatars_sorted:
    ...
    if cambio_max:
        # Normaliza el valor absoluto del cambio para obtener
        # la intensidad del color
        if cambio_max == 0:
            color_intensity = 0
        else:
            color_intensity = 0.2 + 0.8 * abs(cambio) / cambio_max
        ...
        # Dibuja una flecha que va del primer valor al segundo,
        # con un color que depende del cambio y con un grosor mayor
        ax.arrow(
            media_ronda_1, i, cambio, 0, head_width=0.15, head_length=0.15,
            linewidth=5, fc=color, ec=color
        )

    # Dibuja un punto en el valor de inicio
    ax.plot(media_ronda_1, i, 'o', color='black', markersize=10)

    # Dibuja las puntuaciones medias que los otros avatars han dado
    # al avatar considerado
    compatibles = datos['compatibilidades'][avatar]['ronda_1']
    puntuaciones_medias = [
        datos['rondas']['ronda_1'].loc[comp].drop(comp).mean()
        for comp in compatibles
    ]

    if puntuaciones_medias:
        for puntuacion in puntuaciones_medias:
            # Dibuja una 'x' en la posición de cada puntuación media de
            # los avatars compatibles
        ...
        # Dibuja una 'X' más grande en la posición de la media de
        # las puntuaciones medias de los avatars compatibles
...
for i, avatar in enumerate(avatars_sorted):
    ...
    ib.image.axes = ax
    ab = AnnotationBbox(
        ib, (0, i), frameon=False, xybox=(-30, 0), xycoords='data',
        boxcoords='offset points'
    )
    ax.add_artist(ab)
    # Dibujar líneas grises delgadas y discontinuas correspondientes
    # a cada avatar
...
# Establecer el intervalo de los ticks en el eje x
...
# Guardar la figura
plt.savefig('fig/avatar_move_mitjanes.png', bbox_inches='tight')
...
for avatar in avatars_sorted:
    # Usa el diccionario para obtener el índice correcto
    i = index_dict[avatar]
    # Calcula la puntuación que el avatar ha dado a sí mismo
    # en las rondas 1 y 2
...
for avatar in avatars_sorted:
    ...
    # Dibuja una flecha que va del primer valor al segundo, con un color que
    # depende del cambio y con un grosor mayor
    ax.arrow(
        puntuacion_ronda_1, i, cambio, 0, head_width=0.15, head_length=0.15,
        linewidth=5, fc=color, ec=color
    )
    ...
    puntuaciones_comp = [
        rondas['ronda_1'].loc[comp, avatar]
        for comp in compatibilidades[avatar]['ronda_1']
    ]
...
# Guardar la figura
plt.savefig('fig/avatar_move_propies.png', bbox_inches='tight')
...
def plot_avatar_compatibles(avatar, ronda):
    ...
    ib.image.axes = ax
    ab = AnnotationBbox(
        ib, (media_avatar, y_point+0.05), frameon=False, xybox=(0, -30),
    ...
    medias_avatars = [
        (avatar_comp, datos['rondas'][ronda].loc[avatar_comp].mean())
        for avatar_comp in datos['compatibilidades'][avatar][ronda]
    ]
    ...
    for avatar_comp, media_avatar_comp in medias_avatars:
        ...
        ib.image.axes = ax
        ab = AnnotationBbox(
            ib, (media_avatar_comp, y_pos), frameon=False, xybox=(0, -10),
            xycoords='data', boxcoords='offset points'
        )
        ax.add_artist(ab)
        ...
        arco = Arc(
            (centro, y_pos), radio*2, radio*2, theta1=0, theta2=180,
    ...
    for avatar_comp in avatars:
        ...
        if min_medias <= media_avatar_comp <= max_medias:
            ax.text(
                media_avatar_comp, y_point-0.15, '?', color='red', fontsize=20,
                ha='center'
            )
    ...
    # Mostrar más ticks en el eje x
    ax.set_xticks(np.arange(min(todas_las_medias), max(todas_las_medias)+1))
    x_min = round(min(todas_las_medias), 1) - 0.2
    # Agregamos un pequeño incremento al valor máximo
    x_max = round(max(todas_las_medias), 1) + 0.2

    # Use np.linspace to create an array of evenly spaced values
    ...
    ax.set_title(
        f'Compatible Avatars in Round {numero_ronda} for avatar {avatar}\n'
        'Average Scores',
        fontsize=16
    )

    # Guardar la figura con el nombre deseado
    fig.savefig(f'fig/compatibles_{avatar}_{ronda}.png')
...
def plot_avatar_compatibles_subj(avatar, ronda):
    ...
    ib.image.axes = ax
    ab = AnnotationBbox(
        ib, (puntuacion_avatar, y_point+0.05), frameon=False, xybox=(0, -30),
        xycoords='data', boxcoords='offset points'
    )
    ax.add_artist(ab)
    ...
    puntuaciones_avatars = [
        (avatar_comp, datos['rondas'][ronda].loc[avatar_comp].loc[avatar_comp])
        for avatar_comp in datos['compatibilidades'][avatar][ronda]
    ]
    ...
    for avatar_comp, puntuacion_avatar_comp in puntuaciones_avatars:
        ...
        if counter[puntuacion_avatar_comp] > 1:
            print(
                'Intentamos dispersar en la posición ',
                puntuacion_avatar_comp
            )
            seen[puntuacion_avatar_comp] = seen.get(puntuacion_avatar_comp, 0)
            seen[puntuacion_avatar_comp] += 1
        ...
        ib.image.axes = ax
        ab = AnnotationBbox(
            ib, (puntuacion_avatar_comp, y_pos), frameon=False, xybox=(0, -10),
            xycoords='data', boxcoords='offset points'
        )
        ax.add_artist(ab)
    ...
    min_puntuaciones = min(todas_las_puntuaciones)
    max_puntuaciones = max(todas_las_puntuaciones)
    ...
    for avatar_comp in avatars:
        ...
        puntuacion_avatar_comp = datos['rondas'][ronda].loc[avatar_comp]
        puntuacion_avatar_comp = puntuacion_avatar_comp.loc[avatar_comp]

        if min_puntuaciones <= puntuacion_avatar_comp <= max_puntuaciones:
            ax.text(
                puntuacion_avatar_comp, y_point-0.05, '?', color='red',
                fontsize=20, ha='center'
            )

    ax.set_xlim(
        min(todas_las_puntuaciones) - 0.5, max(todas_las_puntuaciones) + 0.5
    )

    y_max = (
        (max(todas_las_puntuaciones) - min(todas_las_puntuaciones))/2.
        + y_point + 0.2
    )
    ax.set_ylim(0.8, y_max)
    ax.set_yticks([])
    ...
    # Mostrar más ticks en el eje x
    ax.set_xticks(
        np.arange(min(todas_las_puntuaciones), max(todas_las_puntuaciones)+1)
    )
    ax.set_xticklabels(
        np.arange(min(todas_las_puntuaciones), max(todas_las_puntuaciones)+1)
    )
    # Poner los números en el eje x

    numero_ronda = ronda.split('_')[1]  # extrae el número de la ronda
    ax.set_title(
        f'Compatible Avatars in Round {numero_ronda} for avatar {avatar}\n'
        f'Scores given by avatar {avatar}',
        fontsize=16
    )

    # Guardar la figura con el nombre deseado
    fig.savefig(f'fig/compatibles_{avatar}_{ronda}_SUBJ.png')
...
def generar_grafo(ronda, datos):
    ...
    for i, (avatar, media) in enumerate(medias.items()):
        ...
        ab = AnnotationBbox(
            ib, (media, 0), frameon=False, xybox=(0, yoffset), xycoords='data',
            boxcoords='offset points'
        )
        ax.add_artist(ab)
    ...
    for avatar_principal, compats in datos['compatibilidades'].items():
        if compats.keys():
            for avatar_compat in compats[ronda]:
                ...
                # Dibujar el arco en el sentido correcto
                # y con el color correcto
                if media_principal < media_compat:
                    color = 'blue'
                    arco = Arc(
                        (centro, 0), radio*2, radio*2, theta1=0, theta2=180,
                        edgecolor=color, linewidth=2, alpha=1-(2*radio/max_dif)
                    )
                else:
                    color = 'red'
                    arco = Arc(
                        (centro, 0), radio*2, radio*2, theta1=180, theta2=360,
                        edgecolor=color, linewidth=2, alpha=1-(2*radio/max_dif)
                    )
                ax.add_patch(arco)
    ...
    ax.set_ylim(
        (medias.min()-medias.max())/2.-0.2, (medias.max()-medias.min())/2.+0.2
    )
    ...
    # Guardar la figura con el nombre deseado
    fig.savefig(f'fig/xarxa_compatibles_{ronda}.png')
